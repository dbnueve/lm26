...
const PlayoffsPage = ({ userTeam, onPlayPlayoffMatch, showToast, onSplitEnd, onSimulateSeason, league = "LEC" }) => {
  ...
  if (loading) return <div style={{ padding: 40, textAlign: "center", color: "var(--text-secondary)" }}>Chargement...</div>;
    ...
    return (
      <div className="animate-slide-up">
        <h2 className="font-heading" style={{ fontSize: 32, marginBottom: 24 }}>
          <Trophy size={32} style={{ marginRight: 12, color: "var(--secondary)" }} />
          Playoffs {league}
        </h2>
        <div className="card" style={{ textAlign: "center", padding: 60 }}>
          <Trophy size={64} style={{ color: "var(--text-secondary)", marginBottom: 24 }} />
  ...
  const MatchCard = ({ match }) => {
    ...
    return (
      <div style={{
        background: "var(--bg-dark)", borderRadius: 8, padding: 14,
        border: isUserMatch && !match.completed ? "2px solid var(--primary)" : "1px solid var(--border-subtle)",
        opacity: !match.completed && !canPlay ? 0.5 : 1,
      }}>
        <div style={{ fontSize: 10, color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ flex: 1, textAlign: "center" }}>
            ...
            <div style={{ fontSize: 11, marginTop: 4, color: t1win ? "var(--success)" : "var(--text-secondary)" }}>
              {match.team1_data?.abbr}
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 14px", background: "var(--surface)", borderRadius: 4 }}>
          ...
          <div style={{ flex: 1, textAlign: "center" }}>
            ...
            <div style={{ fontSize: 11, marginTop: 4, color: t2win ? "var(--success)" : "var(--text-secondary)" }}>
  ...
  const Section = ({ title, rounds, color }) => {
    ...
    return (
      <div>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 2, color: color || "var(--text-secondary)", marginBottom: 8, textTransform: "uppercase" }}>
          {title}
        </div>
  ...
  return (
    <div className="animate-slide-up">
      <h2 className="font-heading" style={{ fontSize: 32, marginBottom: 16 }}>
        <Trophy size={28} style={{ marginRight: 10, color: "var(--secondary)" }} />
      ...
      {playoffsData.champion && (
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          <Trophy size={64} weight="fill" style={{ color: "var(--secondary)", marginBottom: 12 }} />
          <h2 className="font-heading" style={{ fontSize: 32, color: "var(--secondary)" }}>CHAMPION {league} 2026</h2>
      ...
      <div className="card" style={{ padding: 14, marginBottom: 20 }}>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          <span style={{ fontSize: 12, color: "var(--text-secondary)", marginRight: 4 }}>Qualifiés :</span>
          {playoffsData.qualified_teams?.map(t => (
            <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 12px",
              background: t.id === userTeam.id ? "var(--primary)" : t.bracket === "UB" ? "rgba(0,122,255,0.15)" : "rgba(255,51,102,0.12)",
              ...
              <span>{t.abbr}</span>
              <span style={{ fontSize: 10, color: t.id === userTeam.id ? "#000" : "var(--text-secondary)" }}>
                {t.bracket}
              </span>
            </div>
          ))}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "var(--primary)", letterSpacing: 1, textTransform: "uppercase", borderBottom: "2px solid var(--primary)", paddingBottom: 6 }}>
            Upper Bracket
          </div>
      ...
      {(playoffsData.matches?.some(m => m.round === "grand_final")) && (
        <div style={{ marginTop: 24, maxWidth: 480, margin: "24px auto 0" }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "var(--secondary)", textAlign: "center",
            letterSpacing: 2, textTransform: "uppercase", marginBottom: 12, borderBottom: "2px solid var(--secondary)", paddingBottom: 6 }}>
            Grande Finale
          </div>
