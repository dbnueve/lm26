...
const PlayerDetailModal = ({ player, onClose, actions = null }) => {
  ...
  const getPotentialColor = (v) => v >= 90 ? "var(--success)" : v >= 80 ? "var(--amber)" : "var(--text-2)";
  const teamLabel = player.current_team || player.team_abbr || "";
  const leagueLabel = player.league || "";

  const attrs = [
    { label: "Mécanique",   key: "mechanics",  color: "var(--accent)" },
    { label: "Game Sense",  key: "game_sense", color: "#a78bfa" },
    { label: "Teamwork",    key: "teamwork",   color: "var(--amber)" },
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 4, width: 560, maxWidth: "95vw", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 24px 64px rgba(0,0,0,0.6)" }}
      >
        {/* Hero banner with optional splash background */}
        <div style={{ position: "relative", background: "linear-gradient(135deg, var(--surface-1) 0%, #1a1a2e 100%)", padding: "24px 24px 0", borderBottom: "1px solid var(--border)", overflow: "hidden" }}>
          
          <div style={{ position: "relative", zIndex: 1 }}>
            <button onClick={onClose} style={{ position: "absolute", top: 0, right: 0, background: "none", border: "none", color: "var(--text-2)", cursor: "pointer", padding: 4 }}>
              <X size={20} />
            </button>
            <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
              <div style={{ width: 72, height: 72, borderRadius: "50%", overflow: "hidden", flexShrink: 0, background: "var(--surface-1)", border: "2px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 700, color: "var(--accent)" }}>
                {imgUrl
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: "flex", gap: 28, flexWrap: "wrap" }}>
                  {[
                    { label: "Rating",    value: player.rating,                   color: "var(--accent)" },
                    { label: "Potentiel", value: player.potential,                color: getPotentialColor(player.potential), star: player.potential >= 90 },
                    { label: "KDA",       value: player.kda?.toFixed(2) ?? "—",   color: "var(--amber)" },
                    { label: "CS/min",    value: player.cs_min?.toFixed(1) ?? "—",color: "#60a5fa" },
                  ].map(({ label, value, color, star }) => (
                    <div key={label}>
                      <div style={{ fontSize: 11, color: "var(--text-2)", textTransform: "uppercase", letterSpacing: 1 }}>{label}</div>
          ...
          {tab === "overview" && (
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {[
                  { label: "Moral",   value: player.moral ?? 0,    color: (player.moral ?? 0) > 70 ? "var(--success)" : (player.moral ?? 0) > 40 ? "var(--amber)" : "var(--danger)" },
                  { label: "Fatigue", value: player.fatigue ?? 0,  color: (player.fatigue ?? 0) > 70 ? "var(--danger)" : (player.fatigue ?? 0) > 40 ? "var(--amber)" : "var(--success)" },
                ].map(({ label, value, color }) => (
                  <div key={label} style={{ background: "var(--surface-1)", borderRadius: 6, padding: "12px 14px" }}>
              ...
              {/* Attributs */}
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "var(--text-2)", marginBottom: 12 }}>Attributs</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                {[
                  ...
                  <div key={label} style={{ background: "var(--surface-1)", borderRadius: 4, padding: "10px 12px", textAlign: "center" }}>
              ...
              {sigChamp && (
                <div style={{ display: "flex", alignItems: "center", gap: 14, background: "var(--surface-1)", borderRadius: 6, padding: "12px 16px", border: "1px solid var(--border)" }}>
                  <img loading="lazy"
                  ...
                  />
                  <div>
                    <div style={{ fontSize: 11, color: "var(--text-2)", textTransform: "uppercase", letterSpacing: 1 }}>Champion signature</div>
          ...
          {tab === "stats" && (
            <>
              ...
              {player.match_history?.length > 0 ? (
                <div>
                  <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "var(--text-2)", marginBottom: 10 }}>
                    Historique des matchs
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {[...player.match_history].reverse().map((m, i) => {
                      const champKey = m.champion ? toDDragonKey(m.champion) : null;
                      const scoreColor = m.score >= 8 ? "var(--amber)" : m.score >= 6 ? "var(--success)" : m.score >= 4 ? "var(--text-1)" : "var(--danger)";
                      return (
                        <div key={i} style={{
              ...
              {player.champion_pool?.length > 0 ? (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
                  {player.champion_pool.map((champ, i) => {
                    ...
                    const rateColor = i === 0 ? "var(--amber)" : i === 1 ? "var(--accent)" : "var(--text-2)";
                    return (
                      <div key={i} style={{ background: "var(--surface-1)", borderRadius: 6, overflow: "hidden", border: "1px solid var(--border)" }}>
                        <div style={{ position: "relative", height: 90, overflow: "hidden" }}>
                          ...
                          <img loading="lazy"
                            ...
                            alt=""
                            style={{ position: "absolute", bottom: 6, left: 6, width: 24, height: 24, borderRadius: 4, border: "1px solid var(--border)" }}
                            onError={e => { e.currentTarget.style.display = "none"; }}
                          />
                        </div>
                  ...
                  <div style={{ marginTop: 8 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                      {sorted.map(([champ, { games, wins }]) => {
                        ...
                        const wrColor = wr >= 60 ? "var(--success)" : wr >= 45 ? "var(--amber)" : "var(--danger)";
                        return (
                          <div key={champ} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 10px", background: "var(--surface-1)", borderRadius: 6 }}>
                            <img loading="lazy"
                              src={`https://ddragon.leagueoflegends.com/cdn/${_ddVersion}/img/champion/${key}.png`}
                              alt={champ}
                              style={{ width: 28, height: 28, borderRadius: 4, flexShrink: 0 }}
                            ...
                            <div style={{ fontSize: 12, color: "var(--text-2)" }}>{games}G</div>
